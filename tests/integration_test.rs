...
async fn test_server_initialization_simulation() {
    ...
    match result {
        ...
        Err(e) => {
            panic!(
                "❌ SIMULATION MODE: Server initialization failed unexpectedly: {}",
                e
            );
        }
    }
...
async fn test_connection_mode_detection() {
    ...
    assert!(
        sim_result.is_ok(),
        "Simulation mode should always initialize successfully"
    );
...
async fn test_timeline_operations_simulation() {
    ...
    server
        .handle_tool_call("create_project", Some(project_args))
        .await
    ...
    let result = server
        .handle_tool_call("create_timeline", Some(timeline_args))
        .await;
    match result {
        Ok(response) => {
...
async fn test_media_operations_simulation() {
    ...
    let result = server
        .handle_tool_call("import_media", Some(media_args))
        .await;
    match result {
        Ok(response) => {
...
async fn test_color_wheel_operations_simulation() {
    ...
    let result = server
        .handle_tool_call("set_color_wheel_param", Some(wheel_args))
        .await;
    match result {
        Ok(response) => {
...
async fn test_timeline_item_transform_simulation() {
    ...
    let result = server
        .handle_tool_call("set_timeline_item_transform", Some(transform_args))
        .await;
    match result {
        Ok(response) => {
...
async fn test_keyframe_operations_simulation() {
    ...
    let result = server
        .handle_tool_call("add_keyframe", Some(keyframe_args))
        .await;
    match result {
        Ok(response) => {
...
async fn test_render_operations_simulation() {
    ...
    let result = server
        .handle_tool_call("add_to_render_queue", Some(render_args))
        .await;
    match result {
        Ok(response) => {
...
async fn test_export_project_simulation() {
    ...
    let result = server
        .handle_tool_call("export_project", Some(export_args))
        .await;
    match result {
        Ok(response) => {
